...
public class ControlForm extends CanvasConsumer {
	...
	public ControlForm(Controller ctl) {
		...
		yCaption  = cfFont.getHeight() + 3;
                ...
                int sz = controller.cScreen.CH >= 220 ? 
				         ...
				         (controller.cScreen.CH >= 500 ? 128 : 64)
				     : 48)
			         : 32)
		             : 16;
	...
	public void resetIconSize(int sz, boolean reload) {
		...
		while (i < 5) { 	// 16,32,48,64,128
			...
			if (sz == 128) {  	// try to find image of different size 128->64->48->32->16->128
				sz = 64;
			} else if (sz == 64) {
				sz = 48;
			} else if (sz == 48) {
		...
		boolean oneMore = false;
        	...
        	if (name.equals("default")) {
                	...
                	newSkin = SK_BOTTOMLINE;
                        newSplit = false;    // allow split only in default mode
                }
	...
	public void redrawIcons() {
 		//System.out.println("ControlForm.redrawIcons");
                int shift = (split ? (icSize>>1) : 0);
		int xShift = xCorner + shift;
		int yShift = yCorner + shift;
	...
	public int getButton(int x, int y) {
                ...
                int iStep = split ? (icSize<<1) : icSize;
	...
	public void commandAction(Command cmd, Displayable d) {
		...
		try {	
			int w = controller.cScreen.CW;
                        ...
                        int iSize = split ? (icSize<<1) : icSize;
